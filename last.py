...
def assign_ranges_to_machines(machines: Dict[str, Dict], orders_df: pd.DataFrame) -> Dict[str, Dict]:
    ...
    for machine_name, machine_info in machines.items():
        ...
        available_ranges = ranges[machine_type]
        
        max_demand_range = max(available_ranges, key=lambda r: range_demand[machine_type][r])
        
        machine_info['min_cross_section'] = max_demand_range[0]
        machine_info['max_cross_section'] = max_demand_range[1]
        
        range_demand[machine_type][max_demand_range] -= 1
...
def calculate_setup_time(current_setup: Dict, new_setup: Dict, machine_type: str, wire_changed: bool) -> float:
    ...
    if machine_type.startswith('AC'):
        ...
        if current_setup['seals_right'] != new_setup['seals_right']:
            setup_time += constants['AC_SEAL_CHANGE_TIME']
        if current_setup['terminals'] != new_setup['terminals']:
            setup_time += constants['AC_TERMINAL_CHANGE_TIME']
    else:  # KOMAX machines
        if current_setup['seals_left'] != new_setup['seals_left'] :
        ...
        if current_setup['seals_right'] != new_setup['seals_right']:
            setup_time += constants['KOMAX_SEAL_CHANGE_TIME']
        if current_setup['terminals'] != new_setup['terminals']:
            setup_time += constants['KOMAX_TERMINAL_CHANGE_TIME']*2
...
def calculate_assignment_score(machine_info: Dict, group: pd.DataFrame, new_total_time: float, max_terminals: int, avg_machine_time: float) -> float:
    ...
    setup_change_score = (seal_changes + terminal_changes) / (max_terminals + get_constants()['MAX_SEALS_AC96'] * 2)
    balance_score = abs(new_total_time - avg_machine_time) / get_constants()['MAX_DAILY_MINUTES']

    return setup_change_score + balance_score
...
def can_assign_to_machine(order: pd.DataFrame, machine_info: Dict, max_terminals: int = get_constants()['INITIAL_MAX_TERMINALS'], 
    ...
    if cross_section < machine_info['min_cross_section'] or cross_section >= machine_info['max_cross_section']:
        return False, float('inf'), f"Cross-section out of range for this machine"
    ...
    setup_time = calculate_setup_time(machine_info['last_setup'], new_setup, machine_type, wire_changed) 
...
def assign_group_to_machine(group: pd.DataFrame, machine_info: Dict, machine: str):
    ...
    new_setup_time = calculate_setup_time(machine_info['last_setup'], new_setup, machine_type, wire_changed)
    
    new_processing_time = machine_info['processing_time'] + group[time_column].sum()
    new_total_time = calculate_total_time(new_processing_time, new_setup_time)
...
def load_balancing_pass(machine_data: Dict[str, Dict], orders_df: pd.DataFrame, threshold: float = get_constants()['LOAD_BALANCE_THRESHOLD']):
    avg_load = sum(m['total_time'] for m in machine_data.values()) / len(machine_data)
    overloaded = [m for m in machine_data.values() if m['total_time'] > avg_load * (1 + threshold)]
    underloaded = [m for m in machine_data.values() if m['total_time'] < avg_load * (1 - threshold)]
    
    for over_m in overloaded:
        for under_m in underloaded:
            reassignable_orders = [o for o in over_m['orders'] if can_assign_to_machine(orders_df[orders_df['N'] == o], under_m)[0]]
            for order in reassignable_orders:
                if (over_m['total_time'] - avg_load > avg_load - under_m['total_time']):
                    if reassign_order(order, over_m, under_m, orders_df):
                        break  # Stop if reassignment was successful
...
def reassign_order(order: int, from_machine: Dict, to_machine: Dict, orders_df: pd.DataFrame):
    ...
    new_total_time = calculate_total_time(to_machine['processing_time'] + order_data[time_column].sum(), to_machine['setup_time'])
    ...
    update_machine_after_reassign(to_machine, order_data, remove=False)
    return True

def update_machine_after_reassign(machine: Dict, order_data: pd.DataFrame, remove: bool):
    ...
    if remove:
        ...
        machine['processing_time'] += time_change
        machine['seals_left'].update(set(order_data['Seal1Key']) - {-1})
        machine['seals_right'].update(set(order_data['Seal2Key']) - {-1})
        machine['terminals'].update(set(order_data['Terminal1Key']) | set(order_data['Terminal2Key']) - {-1})
...
def swap_machine_ranges(machine_data: Dict[str, Dict], orders_df: pd.DataFrame):
    machine_types = set(m['type'] for m in machine_data.values())
    for machine_type in machine_types:
        type_machines = [m for m in machine_data.values() if m['type'] == machine_type]
        for i in range(len(type_machines)):
            for j in range(i + 1, len(type_machines)):
                m1, m2 = type_machines[i], type_machines[j]
                try:
                    if swap_improves_balance(m1, m2, orders_df):
                        swap_ranges(m1, m2)
                except Exception as e:
                    print(f"Error while swapping ranges: {e}")
                    continue
    return machine_data

def swap_improves_balance(m1: Dict, m2: Dict, orders_df: pd.DataFrame) -> bool:
    current_imbalance = abs(m1['total_time'] - m2['total_time'])
    
    # Ensure we're using 'N' column for order identification
    orders_df_indexed = orders_df.set_index('N')
    
    # Simulate swap
    m1_new_orders = [o for o in m2['orders'] if o in orders_df_indexed.index and 
                     m1['min_cross_section'] <= orders_df_indexed.loc[o, 'CrossSection'] < m1['max_cross_section']]
    m2_new_orders = [o for o in m1['orders'] if o in orders_df_indexed.index and 
                     m2['min_cross_section'] <= orders_df_indexed.loc[o, 'CrossSection'] < m2['max_cross_section']]
    
    m1_new_time = sum(orders_df_indexed.loc[o, f"Time_{m1['type']}"] for o in m1_new_orders if o in orders_df_indexed.index)
    m2_new_time = sum(orders_df_indexed.loc[o, f"Time_{m2['type']}"] for o in m2_new_orders if o in orders_df_indexed.index)
    
    new_imbalance = abs(m1_new_time - m2_new_time)
    
    return new_imbalance < current_imbalance

def swap_ranges(m1: Dict, m2: Dict):
    m1['min_cross_section'], m2['min_cross_section'] = m2['min_cross_section'], m1['min_cross_section']
    m1['max_cross_section'], m2['max_cross_section'] = m2['max_cross_section'], m1['max_cross_section']
...
def optimize_with_load_balancing(sorted_groups: List[Tuple[Any, pd.DataFrame]], orders_df: pd.DataFrame, min_machines: int):
    ...
    for num_machines in range(min_machines, len(available_machines) + 1):
        ...
        for _ in range(get_constants()['MAX_BALANCE_ITERATIONS']):
            # Load balancing pass
            machine_data = load_balancing_pass(machine_data, orders_df)
            
            # Unassigned order resolution
            machine_data, unassigned_orders = resolve_unassigned_orders(machine_data, unassigned_orders, orders_df)
            
            # Machine range swapping
            machine_data = swap_machine_ranges(machine_data, orders_df)
        
        # Analyze unassigned orders and add a suitable machine if possible
        if unassigned_orders:
            machine_data, unassigned_orders = add_machine_for_unassigned(machine_data, unassigned_orders, orders_df, available_machines)
        ...
        solution = {
            ...
            'constraint_type': 'balanced'
        }
        all_solutions.append(solution)

        print(f"Efficiency: {efficiency:.2f}, Unassigned orders: {len(unassigned_orders)}")

        if len(unassigned_orders) == 0:
            print(f"Found optimal solution with {len(machine_data)} machines and efficiency {efficiency:.2f}")
            break
...
def analyze_solution(solution, data):
    ...
    for machine, info in solution['machine_data'].items():
        if info['total_time'] > 0:
            cph_2, ae, pe, total_quantity = calculate_cph_and_pe(info, data)
            oee = ae * (pe / 100)  # Calculate OEE
            ...
            machine_analysis.append({
                ...
                'Unique Seals Left': len(info['seals_left'] - {-1}),
                'Unique Seals Right': len(info['seals_right'] - {-1}),
                'Unique Terminals': len(info['terminals'] - {-1}),
    ...
    machine_df = pd.DataFrame(machine_analysis)
    ...
    with tab1:
        ...
        if solution['unassigned']:
            ...
            unassigned_df = data[data['N'].isin(solution['unassigned'])]
    ...
    with tab2:
        ...
        analyze_components(solution, data)
...
def optimization_page():
    ...
    if machines_file is not None:
        ...
        st.write("Available machine types:", set(info['type'] for info in MACHINES.values()))
